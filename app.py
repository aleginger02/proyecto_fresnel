...
def obtener_perfil_topografico(lat1, lon1, lat2, lon2, n_puntos=100):
    ...
    try:
        ...
        elevaciones = [
            resultado["elevation"] for resultado in datos["results"]
        ]
        return np.array(elevaciones)
    except Exception as e:
        st.warning(f"Error al obtener elevación: {e}. Usando terreno plano.")
        return np.full(n_puntos, 2500.0)  # Cota base promedio por defecto


def analizar_enlace(
    lat1, lon1, h1, lat2, lon2, h2, freq_hz, n_puntos=100, criterio=0.6
):
    ...
    elevaciones_reales = obtener_perfil_topografico(
        lat1, lon1, lat2, lon2, n_puntos
    )
    alt_suelo_A = elevaciones_reales[0]
    alt_suelo_B = elevaciones_reales[-1]

    linea_vista = (alt_suelo_A + h1) + (
        (alt_suelo_B + h2) - (alt_suelo_A + h1)
    ) * (distancias / d_total)
    bulto = (d1 * d2) / (2 * K_FACTOR * R_TIERRA)
    obstaculo_total = elevaciones_reales + bulto

    wavelength = C_LUZ / freq_hz
    # Evitar división por cero en d_total
    ...
    return {
        "d_total_km": d_total / 1000,
        "distancias_km": distancias / 1000,
        "linea_vista": linea_vista,
        "obstaculo_total": obstaculo_total,
        "f1": f1,
        "despeje": despeje,
        "idx_critico": idx_critico,
        "despeje_min_pct": despeje_min_pct,
...
def generar_pdf(r):
    ...
    pdf.cell(
        0, 10, "Diagnostico Zona de Fresnel - ITU-R P.526", ln=True, align="C"
    )
    ...
    pdf.cell(
        0,
        6,
        "Estudiante: Laura Maria Montano Poveda | Codigo: 20242678048",
        ln=True,
        align="C",
    )
    pdf.ln(10)
    ...
    pdf.cell(
        0,
        6,
        f"- Distancia total del enlace: {r['d_total_km']:.2f} km",
        ln=True,
    )
    pdf.cell(
        0,
        6,
        f"- Longitud de onda (lambda): {r['wavelength']*100:.2f} cm",
        ln=True,
    )
    pdf.cell(
        0,
        6,
        f"- Radio maximo de la 1ra Zona de Fresnel (F1): {r['f1_max']:.2f} m",
        ln=True,
    )
    pdf.cell(
        0, 6, f"- Despeje minimo encontrado: {r['despeje_min_pct']:.1f}%", ln=True
    )
    pdf.cell(
        0,
        6,
        f"- Criterio de evaluacion exigido: >= {r['criterio']*100:.0f}%",
        ln=True,
    )

    estado = (
        "DESPEJE ADECUADO (CUMPLE)"
        if r["cumple_criterio"]
        else "POSIBLE OBSTRUCCION"
    )
...
freq_unit = st.sidebar.selectbox(
    "Unidad:", options=["Hz", "kHz", "MHz", "GHz"], index=3
)

st.sidebar.header("⚙️ Configuración")
criterio = st.sidebar.slider(
    "Criterio despeje F1:",
    min_value=0.1,
    max_value=1.0,
    value=0.6,
...
if st.sidebar.button("📡 Calcular diagnóstico", use_container_width=True):
    ...
    with st.spinner("Descargando relieve topográfico y calculando..."):
        try:
            st.session_state.resultado = analizar_enlace(
                lat1,
                lon1,
                h1,
                lat2,
...
if st.session_state.resultado is not None:
    ...
    # Trazo de la Línea de Vista y Zona de Fresnel
    ax.plot(
        x, r["linea_vista"], color="#1f77b4", lw=2, label="Línea de vista (LOS)"
    )
    ax.fill_between(
        x,
        r["linea_vista"] - r["f1"],
        ...
        color="#1f77b4",
        alpha=0.2,
        label="1ª zona de Fresnel",
    )

    # Trazo del Terreno con Curvatura
    ax.plot(
        x,
        r["obstaculo_total"],
        color="#8B5A2B",
        lw=1.8,
        label="Topografía + Curvatura",
    )
    # Rellenar desde la base del perfil y no desde 0m para mantener la elipse visible
    cota_inferior = min(r["obstaculo_total"]) - 15
    ax.fill_between(
        x, cota_inferior, r["obstaculo_total"], color="#8B5A2B", alpha=0.4
    )
    ...
    ax.plot(
        x[idx_c],
        r["obstaculo_total"][idx_c],
        "ro",
        ms=8,
        zorder=5,
        label=f"Punto crítico ({r['despeje_min_pct']:.0f}% de F1)",
    )

    # Marcadores de las Torres
    ax.scatter([x[0]], [r["h1"]], color="black", marker="^", s=100, zorder=5)
    ax.scatter([x[-1]], [r["h2"]], color="black", marker="^", s=100, zorder=5)

    # Ajuste de Etiquetas de las Torres
    ax.annotate(
        "Torre A",
        (x[0], r["h1"]),
    ...
    # Zoom optimizado en el eje Y para proyectar la forma abombada del elipsoide
    cota_superior = max(r["h1"], r["h2"]) + r["f1_max"] + 20
    ax.set_ylim(cota_inferior, cota_superior)
    ...
    folium.Marker(
        [r["lat1"], r["lon1"]],
        tooltip=f"Torre A ({r['h1']:.1f} m)",
        icon=folium.Icon(color="red"),
    ).add_to(m)
    folium.Marker(
        [r["lat2"], r["lon2"]],
        tooltip=f"Torre B ({r['h2']:.1f} m)",
